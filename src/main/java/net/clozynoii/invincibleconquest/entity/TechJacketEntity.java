...
public class TechJacketEntity extends TamableAnimal implements GeoEntity {
	...
	public TechJacketEntity(EntityType<TechJacketEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		...
		builder.define(TEXTURE, "techjacket");
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public boolean canUse() {
				if (TechJacketEntity.this.getTarget() != null && !TechJacketEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return TechJacketEntity.this.getMoveControl().hasWanted() && TechJacketEntity.this.getTarget() != null && TechJacketEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = TechJacketEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				TechJacketEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 2);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = TechJacketEntity.this.getTarget();
				if (TechJacketEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					TechJacketEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = TechJacketEntity.this.distanceToSqr(livingentity);
					if (d0 < 50) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						TechJacketEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 2);
					}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		TechJacketEntity retval = InvincibleConquestModEntities.TECH_JACKET.get().create(serverWorld);
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(InvincibleConquestModEntities.TECH_JACKET.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
