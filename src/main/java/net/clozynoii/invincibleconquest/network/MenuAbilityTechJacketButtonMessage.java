...
import net.clozynoii.invincibleconquest.procedures.UnlockTechUppercutProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockTechJabProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockNanoSlamProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockEnergyProjectionProcedure;
...
public record MenuAbilityTechJacketButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<MenuAbilityTechJacketButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(InvincibleConquestMod.MODID, "menu_ability_tech_jacket_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, MenuAbilityTechJacketButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, MenuAbilityTechJacketButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new MenuAbilityTechJacketButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<MenuAbilityTechJacketButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final MenuAbilityTechJacketButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = MenuAbilityTechJacketMenu.guistate;
		...
		if (buttonID == 12) {
			...
			UnlockEnergyProjectionProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 13) {

			UnlockTechJabProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 14) {

			UnlockNanoSlamProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 15) {

			UnlockTechUppercutProcedure.execute(world, x, y, z, entity);
		}
	}
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(MenuAbilityTechJacketButtonMessage.TYPE, MenuAbilityTechJacketButtonMessage.STREAM_CODEC, MenuAbilityTechJacketButtonMessage::handleData);
	}
}
