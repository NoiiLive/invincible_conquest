...
import net.clozynoii.invincibleconquest.procedures.UnlockRobotPortalsProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockPortalFlightProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockDimensionalTravelProcedure;
...
public record MenuAbilityPortalButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<MenuAbilityPortalButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(InvincibleConquestMod.MODID, "menu_ability_portal_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, MenuAbilityPortalButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, MenuAbilityPortalButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new MenuAbilityPortalButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<MenuAbilityPortalButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final MenuAbilityPortalButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = MenuAbilityPortalMenu.guistate;
		...
		if (buttonID == 12) {
			...
			UnlockDimensionalTravelProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 13) {

			UnlockRobotPortalsProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 14) {

			UnlockPortalFlightProcedure.execute(world, x, y, z, entity);
		}
	}
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(MenuAbilityPortalButtonMessage.TYPE, MenuAbilityPortalButtonMessage.STREAM_CODEC, MenuAbilityPortalButtonMessage::handleData);
	}
}
