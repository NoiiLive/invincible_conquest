...
import net.clozynoii.invincibleconquest.procedures.UnlockTimeStopProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockMultiHitProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockFlashTimeProcedure;
...
public record MenuAbilitySpeedsterButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<MenuAbilitySpeedsterButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(InvincibleConquestMod.MODID, "menu_ability_speedster_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, MenuAbilitySpeedsterButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, MenuAbilitySpeedsterButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new MenuAbilitySpeedsterButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<MenuAbilitySpeedsterButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final MenuAbilitySpeedsterButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = MenuAbilitySpeedsterMenu.guistate;
		...
		if (buttonID == 12) {
			...
			UnlockMultiHitProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 13) {

			UnlockFlashTimeProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 14) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(MenuAbilitySpeedsterButtonMessage.TYPE, MenuAbilitySpeedsterButtonMessage.STREAM_CODEC, MenuAbilitySpeedsterButtonMessage::handleData);
	}
}
