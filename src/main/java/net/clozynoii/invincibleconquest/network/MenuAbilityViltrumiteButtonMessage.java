...
import net.clozynoii.invincibleconquest.procedures.UnlockStrongPunchProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockSonicClapProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockDragProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockChopProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockBulletSlamProcedure;
...
public record MenuAbilityViltrumiteButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<MenuAbilityViltrumiteButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(InvincibleConquestMod.MODID, "menu_ability_viltrumite_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, MenuAbilityViltrumiteButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, MenuAbilityViltrumiteButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new MenuAbilityViltrumiteButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<MenuAbilityViltrumiteButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final MenuAbilityViltrumiteButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = MenuAbilityViltrumiteMenu.guistate;
		...
		if (buttonID == 12) {
			...
			UnlockSonicClapProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 13) {

			UnlockStrongPunchProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 14) {
		...
		if (buttonID == 15) {
			...
			UnlockChopProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 16) {

			UnlockBulletSlamProcedure.execute(world, x, y, z, entity);
		}
	}
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(MenuAbilityViltrumiteButtonMessage.TYPE, MenuAbilityViltrumiteButtonMessage.STREAM_CODEC, MenuAbilityViltrumiteButtonMessage::handleData);
	}
}
