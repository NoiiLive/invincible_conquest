...
import net.clozynoii.invincibleconquest.procedures.UnlockTelekinesisProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockAtomicSwordProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockAtomicBlastProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockAtomCreationProcedure;
import net.clozynoii.invincibleconquest.procedures.UnlockAirDensityProcedure;
...
public record MenuAbilityAtomButtonMessage(int buttonID, int x, int y, int z, HashMap<String, String> textstate) implements CustomPacketPayload {

	public static final Type<MenuAbilityAtomButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(InvincibleConquestMod.MODID, "menu_ability_atom_buttons"));
	public static final StreamCodec<RegistryFriendlyByteBuf, MenuAbilityAtomButtonMessage> STREAM_CODEC = StreamCodec.of((RegistryFriendlyByteBuf buffer, MenuAbilityAtomButtonMessage message) -> {
		buffer.writeInt(message.buttonID);
		buffer.writeInt(message.x);
		buffer.writeInt(message.y);
		buffer.writeInt(message.z);
		writeTextState(message.textstate, buffer);
	}, (RegistryFriendlyByteBuf buffer) -> new MenuAbilityAtomButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt(), readTextState(buffer)));
	@Override
	public Type<MenuAbilityAtomButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final MenuAbilityAtomButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z, HashMap<String, String> textstate) {
		...
		HashMap guistate = MenuAbilityAtomMenu.guistate;
		...
		if (buttonID == 12) {
			...
			UnlockAtomicBlastProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 13) {

			UnlockTelekinesisProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 14) {

			UnlockAtomicSwordProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 15) {

			UnlockAirDensityProcedure.execute(world, x, y, z, entity);
		}
		if (buttonID == 16) {

			UnlockAtomCreationProcedure.execute(world, x, y, z, entity);
		}
	}
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		InvincibleConquestMod.addNetworkMessage(MenuAbilityAtomButtonMessage.TYPE, MenuAbilityAtomButtonMessage.STREAM_CODEC, MenuAbilityAtomButtonMessage::handleData);
	}
}
